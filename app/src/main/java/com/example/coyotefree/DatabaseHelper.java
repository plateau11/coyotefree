...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void deleteAllNotes(ArrayList<Long> noteids, int size){
        try {
            for (int i = 0; i < size; i++) {
                deleteNoteById(noteids.get(i));
            }
        }catch (Exception e){
            //statements
        }
    ...
    public ArrayList<Long> getAllNoteIds() {
        List<Note> notes = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
    ...
    public Note getNoteById(long noteId) {
        ...
        } catch (SQLException e) {long newRowId = -1;
            // Handle the exception
        } finally {
            db.close();
        }
